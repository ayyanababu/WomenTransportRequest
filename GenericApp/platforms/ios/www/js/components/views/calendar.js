define(function(require){
  ...
  validateDateAndTime: function(date,time){
    ...
    if(date != "" && time != "" && Moment(currentDate).isAfter(date+" "+time))
    {
        ...
        NavigationActions.presentPopup(React.createElement(Msg, {msgLabel: msgLabel, buttons: msgButton, onMsgClick: this._onCancel}));
  ...
  render: function() {
    ...
    return(
      React.createElement("div", {className: "inputBox"}, 
        ...
        React.createElement("input", {type: "date", key: keyDate, className: className+" date"+" icon-Calender", id: "cdate", onChange: this._handleDate, defaultValue: date}), 
        React.createElement("input", {type: "time", key: keyTime, className: className+" time"+" icon-Clock-01", id: "ctime", onChange: this._handleTime, defaultValue: time})
      )
      );
    }
  });
