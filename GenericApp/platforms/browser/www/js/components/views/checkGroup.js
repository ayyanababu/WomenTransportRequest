define(function(require){
  ...
  getContents: function(){
    ...
    for (var i = 0; i < array.length; i++) {
        ...
        {
            content.push(
                React.createElement("input", {key: i, className: className, name: this.props.id, type: "checkbox", onChange: this._handleChange, value: i, checked: true}, array[i].value)
            );
          }
          else {
              content.push(
                  React.createElement("input", {key: i, className: className, name: this.props.id, type: "checkbox", onChange: this._handleChange, value: i}, array[i].value)
            );
        }
      }
